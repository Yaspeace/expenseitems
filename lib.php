...
class JsonApi {
    ...
    function projects() {
      return makeHttpRequest(
        'GET',
        cfg()->moyskladJsonApiEndpointUrl . '/entity/project',
        $this->accessToken);
    }
    ...
    function expenseitems() {
      return makeHttpRequest(
        'GET',
        cfg()->moyskladJsonApiEndpointUrl . '/entity/expenseitem',
        $this->accessToken);
    }
    ...
    function getAllObjects($entity, $url = false) {
      if(!$url)
        $url = cfg()->moyskladJsonApiEndpointUrl . "/entity/$entity";
      $response = makeHttpRequest(
        'GET',
        $url,
        $this->accessToken);
      if(property_exists($response->meta, 'nextHref')) {
        return array_merge($response->rows, this->getAllObjects($entity, $response->meta->nextHref));
      } else {
        return $response->rows;
      }
    }

    function getObjects($entity_name, $getAll = false) {
      if(!$getAll)
        return makeHttpRequest('GET', cfg()->moyskladJsonApiEndpointUrl . "/entity/$entity_name", $this->accessToken);
      else
        return JsonApi::getAllObjects($entity_name);
    }

    function change($entity, $id, $data) {
      return makeHttpRequest(
        'PUT',
        cfg()->moyskladJsonApiEndpointUrl . "/entity/$entity/$id",
        $this->accessToken,
        $data);
    }
    ...
    function search($entity, $q) {
      $response = makeHttpRequest(
        'GET',
        cfg()->moyskladJsonApiEndpointUrl . "/entity/$entity?search=$q",
        $this->accessToken);
...
function jsonApi(): JsonApi {
    if (!isset($GLOBALS['jsonApi']) || !$GLOBALS['jsonApi']) {
